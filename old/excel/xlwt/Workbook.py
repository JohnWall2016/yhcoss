...
class Workbook(object):
    ...
    def add_first_font(self, font):
        self.__styles.add_first_font(font)
    ...
    def inc_str_ref(self, sst_idx):
        self.__sst.inc_str_ref(sst_idx)    
    ...
    def copy_sheet(self, sheetnum_from, sheetname_to):
        from . import Utils
        from .Worksheet import Worksheet
        if not isinstance(sheetname_to, unicode_type):
            sheetname_to = sheetname_to.decode(self.encoding)
        if not Utils.valid_sheet_name(sheetname_to):
            raise Exception("invalid worksheet name %r" % sheetname_to)
        lower_name = sheetname_to.lower()
        if lower_name in self.__worksheet_idx_from_name:
            raise Exception("duplicate worksheet name %r" % sheetname_to)
        self.__worksheet_idx_from_name[lower_name] = len(self.__worksheets)
        self.__worksheets.append(self.get_sheet(sheetnum_from).copy(sheetname_to))
        return self.__worksheets[-1]

    def get_sheet(self, sheetnum) -> Worksheet:
        return self.__worksheets[sheetnum]

    def get_sheet_index_by_name(self, sheetname):
        lower_name = sheetname.lower()
        if not lower_name in self.__worksheet_idx_from_name:
            raise Exception("not found the worksheet %r" % sheetname)
        else:
            return self.__worksheet_idx_from_name[lower_name]

    def del_sheet(self, sheetnum):
        sheetname = ''
        for name, num in self.__worksheet_idx_from_name.items():
            if num > sheetnum:
                self.__worksheet_idx_from_name[name] -= 1
            if num == sheetnum:
                sheetname = name
        del self.__worksheet_idx_from_name[sheetname]
        del self.__worksheets[sheetnum]
        self.__active_sheet = 0
        self.__first_tab_index = 0
        self.__selected_tabs = 0x01
