...
class Row(object):
    ...
    def copy_to(self, indx):
        import copy
        row = copy.copy(self)
        row.__idx = indx
        row.__cells = copy.deepcopy(self.__cells)
        for col in row.__cells:
            cell = row.__cells[col]
            if cell:
                sst_idx = getattr(cell, 'sst_idx', None)
                if sst_idx is not None:
                    self.__parent_wb.inc_str_ref(sst_idx)
                cell.rowx = indx
        return row

    def copy_to_sheet(self, sheet):
        import copy
        row = copy.copy(self)
        row.__parent = sheet
        row.__parent_wb = sheet.get_parent()
        row.__cells = copy.deepcopy(self.__cells)
        for col in row.__cells:
            cell = row.__cells[col]
            if cell:
    ...
    def copy_style_to(self, row):
        row.__height_in_pixels = self.__height_in_pixels
        ...
        row.__has_default_xf_index = self.__has_default_xf_index
        row.__xf_index = self.__xf_index
        row.space_above = self.space_above
        row.space_below = self.space_below
        
        for col in self.__cells:
            fcell = self.__cells[col]
            if fcell:
                style_index = fcell.xf_idx
                tcell = row.__cells.get(col)
                if tcell:
                    tcell.xf_idx = style_index
                else:
                    row.insert_cell(col, BlankCell(row.__idx, col, style_index))

    def move_to(self, indx):
        self.__idx = indx
        for col in self.__cells:
