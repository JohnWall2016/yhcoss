...
import logging
import os
from src.excel.xls.third_party import xlrd
from src.excel.xls.third_party import xlwt
...
class BaseWriter:
    ...
    def workbook(self,rdbook,wtbook_name):
        ...
        rdff = rdbook.font_list[0]
        if rdff:
            wtff = xlwt.Formatting.Font() # first font
            wtff.height = rdff.height
            wtff.italic = rdff.italic
            wtff.struck_out = rdff.struck_out
            wtff.outline = rdff.outline
            wtff.shadow = rdff.outline
            wtff.colour_index = rdff.colour_index
            wtff.bold = rdff.bold #### This attribute is redundant, should be driven by weight
            wtff._weight = rdff.weight #### Why "private"?
            wtff.escapement = rdff.escapement
            wtff.underline = rdff.underline_type #### 
            # wtff.???? = rdff.underline #### redundant attribute, set on the fly when writing
            wtff.family = rdff.family
            wtff.charset = rdff.character_set
            wtff.name = rdff.name
            self.wtbook.add_first_font(wtff)
    ...
    def sheet(self,rdsheet,wtsheet_name):
        ...
        wtsheet.set_page_setup_record(rdsheet.page_setup_record)

        wtsheet.left_margin = rdsheet.page_margins.get('left', wtsheet.left_margin)
        wtsheet.right_margin = rdsheet.page_margins.get('right', wtsheet.right_margin)
        wtsheet.top_margin = rdsheet.page_margins.get('top', wtsheet.top_margin)
        wtsheet.bottom_margin = rdsheet.page_margins.get('bottom', wtsheet.bottom_margin)

        wtsheet.print_centered_horz = rdsheet.print_centered.get('horz',
                                                                 wtsheet.print_centered_horz)
        wtsheet.print_centered_vert = rdsheet.print_centered.get('vert',
                                                                 wtsheet.print_centered_horz)

        wtsheet.footer_str = rdsheet.footer
        wtsheet.header_str = rdsheet.header
