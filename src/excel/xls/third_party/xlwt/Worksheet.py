...
class Worksheet(object):
    ...
    ## Constructor
    #################################################################
    def __init__(self, sheetname, parent_book, cell_overwrite_ok=True):
        self.Row = Row
        self.Column = Column
    ...
    def set_page_setup_record(self, record):
        self.paper_size_code = record[0]
        self.print_scaling = record[1]
        self.start_page_number = record[2]
        self.fit_width_to_pages = record[3]
        self.fit_height_to_pages = record[4]
        self.set_setup_page_options(record[5])
        self.print_hres = record[6]
        self.print_vres = record[7]
        self.header_margin = record[8]
        self.footer_margin = record[9]
        self.copies_num = record[10]

    def set_setup_page_options(self, option):
        self.print_in_rows = option & 0x0001
        self.portrait = option & 0x0002
        self.print_colour = not (option & 0x0008)
        self.print_draft = option & 0x0010
        self.print_notes = option & 0x0020
        self.print_notes_at_end = option & 0x0200
        self.print_omit_errors = (option & 0x0400)
    ...
    def update(self, r, c, label=""):
        self.row(r).update(c, label)
    ...
    def insert_row(self, indx) -> 'Row':
        keys = sorted(self.__rows.keys(), reverse=True)
        for i in keys:
            if i >= indx:
                self.__rows[i+1] = self.__rows[i].move_to(i+1)
                if i+1 > self.last_used_row:
                    self.last_used_row = i+1
                del self.__rows[i]
        newrow = self.row(indx)
        merged_ranges = []
        for r1, r2, c1, c2 in self.__merged_ranges:
            r1 = min(r1, r2)
            r2 = max(r1, r2)
            if r2 < indx:
                merged_ranges.append((r1, r2, c1, c2))
            elif r1 >= indx:
                merged_ranges.append((r1+1, r2+1, c1, c2))
            elif r2 >= indx:
                merged_ranges.append((r1, r2+1, c1, c2))
        self.__merged_ranges = merged_ranges
        return newrow
    ...
    def __print_settings_rec(self):
        ...
        setup_page_options |=  (0x00 & 0x01) << 7
    ...
    def copy(self, newname):
        import copy
        newsheet = copy.copy(self)
        #print(newsheet.__name, newname)
        newsheet.__name = newname
        newsheet.__rows = {}
        for index, row in self.__rows.items():
            newsheet.__rows[index] = row.copy_to_sheet(newsheet)
        #newsheet.__rows = copy.deepcopy(self.__rows)
        newsheet.__cols = {}
        for index, col in self.__cols.items():
            newsheet.__cols[index] = col.copy_to_sheet(newsheet)
        #newsheet.__cols = copy.deepcopy(self.__cols)
        newsheet.__merged_ranges = copy.deepcopy(self.__merged_ranges)
        newsheet.__vert_page_breaks = copy.deepcopy(self.__vert_page_breaks)
        newsheet.__horz_page_breaks = copy.deepcopy(self.__horz_page_breaks)
        newsheet.__flushed_rows = copy.deepcopy(self.__flushed_rows)
        return newsheet

    def get_or_create_row_from(self, newrow_index, fromrow_index):
        if newrow_index > fromrow_index:
            newrow = self.insert_row(newrow_index)
            self.copy_row_style(fromrow_index, newrow_index)
            return newrow
        else:
            return self.row(newrow_index)
