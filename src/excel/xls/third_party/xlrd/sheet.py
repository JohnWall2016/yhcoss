    ...
    def read(self, bk):
        ...
        while 1:
            ...
            if rc == XL_NUMBER:
                ...
                self.page_setup_record = unpack('<8H2dH', data)
            elif rc == XL_FOOTER:
                if data:
                    if bv < BIFF_FIRST_UNICODE:
                         strg = unpack_string(data, 0, bk.encoding or bk.derive_encoding(), lenlen=2)
                    else:
                ...
                self.page_margins['left'], = unpack('<d', data)
            elif rc == XL_RIGHTMARGIN:
                self.page_margins['right'], = unpack('<d', data)
            elif rc == XL_BOTTOMMARGIN:
                self.page_margins['bottom'], = unpack('<d', data)
            elif rc == XL_TOPMARGIN:
                self.page_margins['top'], = unpack('<d', data)
            elif rc == XL_HCENTER:
                self.print_centered['horz'], = unpack('<H', data)
            elif rc == XL_VCENTER:
                self.print_centered['vert'], = unpack('<H', data)
            elif rc == XL_RSTRING:
                rowx, colx, xf_index = local_unpack('<HHH', data[0:6])
                if bv < BIFF_FIRST_UNICODE:
